...
void BezierWing::draw(){
    ...
    // Bezier control points ( 0 = first curve, 1 = second curse )
    ...
    glBegin(GL_POLYGON);
        for(int t = 0;t<this->getPrecision();t++){
            ...
            float xt0 = courbeBezier0[0][0]*std::pow((1-currentT),3)+3*courbeBezier0[1][0]*currentT*std::pow((1-currentT),2)+3*courbeBezier0[2][0]*std::pow(currentT,2)*(1-currentT)+courbeBezier0[3][0]*std::pow(currentT,3);
            float zt0 = courbeBezier0[0][1]*std::pow((1-currentT),3)+3*courbeBezier0[1][1]*currentT*std::pow((1-currentT),2)+3*courbeBezier0[2][1]*std::pow(currentT,2)*(1-currentT)+courbeBezier0[3][1]*std::pow(currentT,3);

            points0Bezier[t][0] = xt0;
            points0Bezier[t][1] = zt0;
            glVertex3f(xt0,0,zt0);
        }

        for(int t = 0;t<this->getPrecision();t++){
            float currentT = (t+0.0f)/this->getPrecision();
            float xt1 = courbeBezier1[0][0]*std::pow((1-currentT),3)+3*courbeBezier1[1][0]*currentT*std::pow((1-currentT),2)+3*courbeBezier1[2][0]*std::pow(currentT,2)*(1-currentT)+courbeBezier1[3][0]*std::pow(currentT,3);
            float zt1 = courbeBezier1[0][1]*std::pow((1-currentT),3)+3*courbeBezier1[1][1]*currentT*std::pow((1-currentT),2)+3*courbeBezier1[2][1]*std::pow(currentT,2)*(1-currentT)+courbeBezier1[3][1]*std::pow(currentT,3);

            points1Bezier[t][0] = xt1;
            points1Bezier[t][1] = zt1;
            glVertex3f(xt1,0,zt1);
        }
    glEnd();
